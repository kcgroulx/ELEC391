...
#include "piano_keymap.h"   /* MIDI_NOTE_MIN, MIDI_NOTE_MAX               */
#include "hal_interface.h"  /* hal_getTick(), UART handle                 */
#include "main.h"           /* huart2                                     */
#include "usart.h"
#include <string.h>         /* memcmp                                     */
#include <stdio.h>          /* snprintf (debug only)                      */
...
uint32_t Midi_receiveUART(uint8_t* buf, uint32_t bufSize, uint32_t timeoutMs)
{
    ...
    status = HAL_UART_Receive(&huart2, lenBytes, 4, timeoutMs);
    ...
    /* Receive file bytes */
    status = HAL_UART_Receive(&huart2, buf, (uint16_t)fileLen, timeoutMs);
    if (status != HAL_OK) return 0;
